...
public class CategoryController {
    ...
    @GetMapping
    @ApiOperation("显示全部列表")
    public Resp<List<CategoryEntity>> queryCategoryByLevelOrPid(@RequestParam(value = "level",defaultValue = "0")Integer level
            , @RequestParam(value = "parentCid",required = false)Long pid){
        List<CategoryEntity> categoryEntities=this.categoryService.queryCategoryByLevelOrPid(level,pid);
