...
public class OrderListener {
    ...
    public void closeOrder(String orderToken){
        ...
        if(this.orderDao.closeOrder(orderToken) == 1) {
            // 2.立马解锁库存
            this.template.convertAndSend("WMS-EXCHANGE", "stock.unlock", orderToken);
        }
    }
    ...
    public void payOrder(String orderToken){
        ...
        if (this.orderDao.payOrder(orderToken) == 1){
            // 2.真正的减库存操作
            this.template.convertAndSend("WMS-EXCHANGE", "stock.minus", orderToken);
            // 3.给用户添加积分 TODO
        }
    }
}
